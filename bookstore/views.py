...
from .models import User, Book, Chat, MakeRequest, Feedback
...
@login_required
def mkreq_form(request):
   	return render(request, 'student/make_request.html')
...
def make_request(request):
	if request.method == 'POST':
		...
		a = MakeRequest.objects.create(title=title, author=author, year=year, publisher=publisher, 
			desc=desc, requested_by=username, user_id=user_id)
		a.save()
		messages.success(request, 'Requested was made successfully')
		return redirect('mkreq_form')
	else:
	    messages.error(request, 'Requested was not made successfully')
	    return redirect('mkreq_form')
...
# def make_request(request):
# 	if request.method == 'POST':
# 		book_id = request.POST['make_request']
# 		current_user = request.user
# 		user_id = current_user.id
# 		username = current_user.username
# 		user_request = username + "  has requested for " + book_id 

# 		a = MakeRequest(make_request=user_request)
# 		a.save()
...
def labook_form(request):
	return render(request, 'lecturer/add_book.html')
...
class LBookListView(LoginRequiredMixin,ListView):
	model = Book
	template_name = 'lecturer/book_list.html'
	context_object_name = 'books'
	paginate_by = 5
...
class LManageBook(LoginRequiredMixin,ListView):
	model = Book
	template_name = 'lecturer/manage_books.html'
	context_object_name = 'books'
	paginate_by = 5
...
class LDeleteRequest(LoginRequiredMixin,ListView):
	model = MakeRequest
	template_name = 'lecturer/make_request.html'
	context_object_name = 'feedbacks'
	paginate_by = 10

	def get_queryset(self):
		return MakeRequest.objects.order_by('-id')


class LViewBook(LoginRequiredMixin,DetailView):
	model = Book
	template_name = 'lecturer/book_detail.html'

	
class LEditView(LoginRequiredMixin,UpdateView):
	model = Book
	form_class = BookForm
	template_name = 'lecturer/edit_book.html'
...
class LDeleteBook(LoginRequiredMixin,DeleteView):
	model = Book
	template_name = 'lecturer/confirm_delete2.html'
	success_url = reverse_lazy('lecturer')
	success_message = 'Data was dele successfully'

class LMakeRequest(LoginRequiredMixin,ListView):
	model = MakeRequest
	template_name = 'lecturer/manage_request.html'
	context_object_name = 'feedbacks'
	paginate_by = 10

	def get_queryset(self):
		print(MakeRequest.objects.order_by('-id'))
		return MakeRequest.objects.order_by('-id')



class LDeleteRequest(LoginRequiredMixin,DeleteView):
	model = MakeRequest
	template_name = 'lecturer/confirm_del.html'
...
def lsearch(request):
    ...
    if( len(data) == 0):
        return redirect('student')
                ...
                if files:
                    return render(request,'lecturer/result.html',{'files':files,'word':word})
                return render(request,'lecturer/result.html',{'files':files,'word':word})
...
class LCreateChat(LoginRequiredMixin, CreateView):
	form_class = ChatForm
	model = Chat
	template_name = 'lecturer/chat_form.html'
...
class LListChat(LoginRequiredMixin, ListView):
	model = Chat
	template_name = 'lecturer/chat_list.html'
...
def create_user_form(request):
    choice = ['1', '0', 'Student', 'Admin', 'Lecturer']
...
def create_user(request):
    ...
    if request.method == 'POST':
            ...
            if userType == "Student":
                ...
                a = User(first_name=first_name, last_name=last_name, username=username, email=email, password=password, is_lecturer=True)
                a.save()
                messages.success(request, 'Lecturer was created successfully!')
                return redirect('aluser')    
            else:
...
class ADeleteRequest(LoginRequiredMixin,DeleteView):
	model = MakeRequest
	template_name = 'dashboard/confirm_del.html'
	success_url = reverse_lazy('amrequest')
	success_message = 'Data was deleted successfully'

# class ADeleteRequest(SuccessMessageMixin, DeleteView):
#     model = User
#     template_name='dashboard/confirm_delete3.html'
#     success_url = reverse_lazy('adreq')
#     success_message = "Data successfully deleted"
...
class AMakeRequest(LoginRequiredMixin,ListView):
	model = MakeRequest
	template_name = 'dashboard/manage_request.html'
	context_object_name = 'feedbacks'
	paginate_by = 10
